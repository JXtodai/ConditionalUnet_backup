...
GRADIENT_ACCUMULATION_STEPS=2       # was 1; halves per-step gradient variance to make weights more robust to outlier batches
LEARNING_RATE=1e-4                  # left as-is: accelerator.load_state restores the optimizer/scheduler state from checkpoint-4000, so changing this has no effect after resume
LR_SCHEDULER="cosine"
LR_WARMUP_STEPS=500
CHECKPOINTING_STEPS=250             # was 500; tighter granularity so any future spike can be rolled back closer
CHECKPOINTS_TOTAL_LIMIT=8           # was 5; compensate for shorter checkpoint interval
SAVE_IMAGES_EPOCHS=2
SAVE_MODEL_EPOCHS=5
DDPM_NUM_STEPS=1000
DDPM_NUM_INFERENCE_STEPS=1000
DATALOADER_NUM_WORKERS=4
MIXED_PRECISION="bf16"              # was fp16; bf16 has fp32 dynamic range, eliminates loss-scaling overflow that caused the step-4260 spike
LOGGER="tensorboard"
THRESHOLD=0.5
...
########## (9) Misc ##########
VALIDATION_SEED=0
# Resume from the latest pre-spike checkpoint (training spiked at step 4260; checkpoint-4000 is the closest healthy state).
RESUME_FROM_CHECKPOINT="/home/jixi/project/genai/output_conditional_unet_aggexp_embed_full_x0pred_cldice5/checkpoint-4000"
