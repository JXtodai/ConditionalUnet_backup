...
def mask_reconstruction_loss(pred_x0, clean_mask, loss_type):
    pred_prob, target_prob = masks_to_probability(pred_x0, clean_mask)

    if loss_type == "bce":
        return F.binary_cross_entropy(pred_prob.clamp(1e-6, 1.0 - 1e-6), target_prob)
    if loss_type == "dice":
        return dice_loss(pred_prob, target_prob)
    if loss_type == "bce_dice":
        bce = F.binary_cross_entropy(pred_prob.clamp(1e-6, 1.0 - 1e-6), target_prob)
                    ...
                    if reconstruction_mask.any():
                        ...
                        if args.mask_reconstruction_loss_weight > 0:
                            recon_loss = mask_reconstruction_loss(
                                recon_pred_x0, recon_clean_mask, args.mask_reconstruction_loss
                            )
