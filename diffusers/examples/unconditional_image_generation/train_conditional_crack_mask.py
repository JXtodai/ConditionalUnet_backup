...
class CrackMaskConditionDataset(Dataset):
    ...
    def _paired_spatial_transform(self, image, mask):
    ...
    def __getitem__(self, index):
        ...
        image, mask = self._paired_spatial_transform(image, mask)
...
def build_validation_grid(concrete_images, gt_masks, pred_masks, binary_masks):
    rows = []
    for concrete_image, gt_mask, pred_mask, binary_mask in zip(concrete_images, gt_masks, pred_masks, binary_masks):
        ...
        rows.append(torch.cat([concrete_image, gt_mask_rgb, pred_mask_rgb, binary_mask_rgb], dim=2))
...
def run_validation(unet, noise_scheduler, validation_batch, epoch, args, accelerator, weight_dtype):
    ...
    for t in noise_scheduler.timesteps:
        ...
        model_input = torch.cat([mask, concrete_image, expansion_map], dim=1)
    ...
    grid = build_validation_grid(concrete_image, gt_mask, pred_mask, binary_mask)
        ...
        accelerator.get_tracker("wandb").log(
            {
                "validation_samples": wandb.Image(
                    sample_path,
                    caption="Columns: concrete image | gt mask | generated mask | thresholded mask",
                ),
                "epoch": epoch,
            },
            step=epoch,
...
def main(args):
    ...
    if args.model_config_name_or_path is None:
        ...
        config.update({"sample_size": args.resolution, "in_channels": 5, "out_channels": 1})
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            model_input = torch.cat([noisy_mask, concrete_image, expansion_map], dim=1)
